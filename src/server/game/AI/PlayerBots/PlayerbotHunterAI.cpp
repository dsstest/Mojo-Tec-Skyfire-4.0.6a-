...
{
    QueryResult result = CharacterDatabase.PQuery("SELECT * FROM character_pet WHERE owner = '%u'",bot->GetGUIDLow());
    ...
    if( PET_SUMMON>0 && !m_petSummonFailed && HasPet(m_bot) )
    {
        ...
        else if(pet->GetHappinessState() != HAPPY) // if pet is hungry
        {
            ...
            for (uint8 slot = INVENTORY_SLOT_ITEM_START; slot < INVENTORY_SLOT_ITEM_END; slot++)
            {
                ...
                if (pItem)
                {
                    const ItemPrototype* const pItemTemplate = pItem->GetProto();
                    if (!pItemTemplate )
                        continue;
