...
bool PlayerbotClassAI::listAuras(Unit *u)
{
    ...
    for(Unit::AuraMap::const_iterator itr = vAuras.begin(); itr!=vAuras.end(); ++itr)
    {
        ...
        sLog->outString("aura = %u %s", spellInfo->Id, name.c_str());
    }
    return true;
};//end listAuras
...
bool PlayerbotClassAI::castSelfCCBreakers (uint32 castList[])
{
    ...
    Player *dTarget = GetPlayerBot();
        ...
        // racial abilities
      if( GetPlayerBot()->getRace() == RACE_BLOODELF && !dTarget->HasAura( R_ARCANE_TORRENT,0 ) && CastSpell( R_ARCANE_TORRENT,dTarget ) ) {
         GetPlayerBot()->Say("Arcane Torrent!", LANG_UNIVERSAL);
    } else if( GetPlayerBot()->getRace() == RACE_HUMAN && (GetPlayerBot()->HasUnitState( UNIT_STAT_STUNNED ) || GetPlayerBot()->HasAuraType( SPELL_AURA_MOD_FEAR ) || GetPlayerBot()->HasAuraType( SPELL_AURA_MOD_DECREASE_SPEED ) || GetPlayerBot()->HasAuraType( SPELL_AURA_MOD_CHARM )) && CastSpell( R_EVERY_MAN_FOR_HIMSELF, GetPlayerBot() ) ) {
        GetPlayerBot()->Say("EVERY MAN FOR HIMSELF!", LANG_UNIVERSAL);
    } else if( GetPlayerBot()->getRace() == RACE_UNDEAD_PLAYER && (GetPlayerBot()->HasAuraType( SPELL_AURA_MOD_FEAR ) || GetPlayerBot()->HasAuraType( SPELL_AURA_MOD_CHARM )) && CastSpell( R_WILL_OF_FORSAKEN, GetPlayerBot() ) ) {
        GetPlayerBot()->Say("WILL OF THE FORSAKEN!", LANG_UNIVERSAL);
    } else if( GetPlayerBot()->getRace() == RACE_DWARF && GetPlayerBot()->HasAuraState( AURA_STATE_DEADLY_POISON ) && CastSpell( R_STONEFORM, GetPlayerBot() ) ) {
        GetPlayerBot()->Say("STONEFORM!", LANG_UNIVERSAL);
    } else if( GetPlayerBot()->getRace() == RACE_GNOME && (GetPlayerBot()->HasUnitState( UNIT_STAT_STUNNED ) || GetPlayerBot()->HasAuraType( SPELL_AURA_MOD_DECREASE_SPEED )) && CastSpell( R_ESCAPE_ARTIST, GetPlayerBot() ) ) {
...
bool PlayerbotClassAI::TakePosition(Unit *followTarget, BotRole bRole, float bDist, float bMinDist, float bMaxDist, float bAngle, Unit *faceTarget)
{
    ...
    {
        const CreatureInfo *creatureInfo = ((Creature*) followTarget)->GetCreatureInfo();
    ...
    if (doFollow)
    {
            ...
            )
        {
            ...
            sLog->outString("Bot[%u] is moving, curDist[%f], bDist[%f], bminDist[%f], bMaxDist[%f], bAngle[%f], InFront[%u]", m_bot->GetGUIDLow(), curDist, bDist,bMinDist, bMaxDist, bAngle, followTarget->HasInArc(M_PI,m_bot));
...
uint8 PlayerbotClassAI::GetHealthPercentRaid(Player *gPlayer, uint8 &countNeedHealing)
{
    ...
    if(!unitList.empty()){
      for (std::list<Unit*>::iterator itr = unitList.begin() ; itr!=unitList.end();++itr) {
        ...
        sLog->outError("me = %s, checked= %s %u [%u / %u]", myname.c_str(), hisname.c_str(), fndHPPercent, tPlayer->GetHealth(), tPlayer->GetMaxHealth());
