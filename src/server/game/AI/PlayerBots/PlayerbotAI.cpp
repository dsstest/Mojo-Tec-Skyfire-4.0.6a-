...
uint32 PlayerbotAI::getSpellId(const char *args, bool master) const
{
    ...
    for(PlayerSpellMap::iterator itr = m_bot->GetSpellMap().begin(); itr != m_bot->GetSpellMap().end(); ++itr)
    {
        ...
        bool usesNoReagents = (spellInfo->Reagent[0] <=  0) ? true : false;
...
uint32 PlayerbotAI::getSpellIdExact(const char *args, bool includePassive, bool master)
{
    ...
    for(PlayerSpellMap::iterator itr = m_bot->GetSpellMap().begin(); itr != m_bot->GetSpellMap().end(); ++itr)
    {
        ...
        if(!spellInfo) continue;
        if(spellInfo->Effect[0] == SPELL_EFFECT_LEARN_SPELL) continue; //This is a learn spell

        std::string name = spellInfo->SpellName;
		name = spellInfo->SpellName[loc];
        if(name.empty()) continue;
        if(strcmp(name.c_str(),namepart.c_str())) continue;
        if(spellInfo->Reagent[0] <=  0 && !foundMatchUsesNoReagents){ foundSpellId = spellId; foundMatchUsesNoReagents = true; }
        else if(spellId > foundSpellId) { foundSpellId = spellId; }
    }
    sLog->outString("PBot Class %u - Found in Search - [%u/%s]", m_bot->getClass(), foundSpellId, namepart.c_str());
    if (foundSpellId > 70000) { 
	sLog->outString("CRITICAL: PBot Class %u - Weird Spell in Search - [%u/%s]", m_bot->getClass(), foundSpellId, namepart.c_str()); 
	}
    return foundSpellId;
}
...
void PlayerbotAI::HandleMasterIncomingPacket(const WorldPacket &packet, WorldSession &masterSession)
{
    switch(packet.GetOpcode())
    {
		...
		{
			//sLog->outString("cmsg_set_selection");
			return;
		}
        ...
        case CMSG_GAMEOBJ_USE:
            {
               ...
               for(PlayerBotMap::const_iterator it = masterSession.GetPlayerBotsBegin(); it != masterSession.GetPlayerBotsEnd(); ++it)
                {
                    Player* const bot = it->second;
//sLog->outString ("gameobject type = %u", obj->GetGoType());
        ...
        case CMSG_GAMEOBJ_REPORT_USE:
            {
                ...
                Object* const pNpc = ObjectAccessor::GetObjectByTypeMask(*masterSession.GetPlayer(), objGUID, TYPEMASK_UNIT|TYPEMASK_GAMEOBJECT);
                if (!pNpc) return;
            /* For all master's bots */
            for (PlayerBotMap::const_iterator it = masterSession.GetPlayerBotsBegin();
                it != masterSession.GetPlayerBotsEnd(); ++it)
...
// TESTING
        ...
        const char *oc = LookupOpcodeName(packet.GetOpcode());
        TellMaster(oc);
        sLog->outString("SMSG opcode: %s", oc);
  // TESTING
    }
}
...
void PlayerbotAI::UseMount() const
{
    ...
    {
// sLog->outString ("PlayerbotAI::UseMount: %s is mounted but %s is not", m_master->GetName(), m_bot->GetName());
        //Player Part
...
//sLog->outString ("master_speed1 = %d", master_speed1);
//sLog->outString ("master_speed2 = %d", master_speed2);
        //Bot Part
        uint32 spellMount = 0;
        for(PlayerSpellMap::iterator itr = m_bot->GetSpellMap().begin(); itr != m_bot->GetSpellMap().end(); ++itr)
        {
        ...
        sLog->outString ("spellMount = %u", spellMount);
   ...
   if (!thingToAttack)
    {
		...
		else sLog->outString ("%s is attacking %s", m_bot->GetName(), thingToAttack->GetName());
        }
    }
...
void PlayerbotAI::DoNextCombatManeuver()
{
        ...
        !m_bot->GetPlayerbotAI()->GetClassAI()->isPulling() )
    {
        ...
        sLog->outString ("current target doesn't make sense so following");
        Follow(*m_master);
        return;
    }
...
sLog->outString ("%s - sitting so feast", m_bot->GetName());
    ...
    {
        sLog->outString ("%s: %s is too far away so following", m_bot->GetName(), m_master->GetName());
        Follow(*m_master);
    }
}
...
bool PlayerbotAI::CastSpell(uint32 spellId, Unit *target, bool checkFirst, bool castExistingAura, bool skipFriendlyCheck, bool skipEquipStanceCheck, bool triggered)
{
    ...
    if (!pSpellInfo)
    {
        sLog->outString("CRITICAL: PBot Class %u - Non-existing Spell in Cast - [%u]", m_bot->getClass(), spellId);
        std::stringstream ss;
...
bool PlayerbotAI::CastSpell(const SpellEntry * pSpellInfo, Unit *target, bool checkFirst, bool castExistingAura, bool skipFriendlyCheck, bool skipEquipStanceCheck, bool triggered)
{
    ...
    if(!pSpellInfo)
    {
        sLog->outString ("%s: Missing spell entry in CastSpell Direct", m_bot->GetName());
    ...
    sLog->outString("Bot [%u] Start Spell [%u] Cast Time [%f]", m_bot->GetGUIDLow(), pSpellInfo->Id, psCastTime);
...
bool PlayerbotAI::CanCast(const SpellEntry * pSpellInfo, Unit *target, bool castExistingAura, bool skipFriendlyCheck, bool skipEquipStanceCheck)
{
    if (!pSpellInfo)
    {
        sLog->outString("CRITICAL: PBot Class %u - Non-existing Spell in CastCheck - Direct SpellEntry", m_bot->getClass());
        TellMaster("Missing spell entry in CastSpell");
        SetIgnoreUpdateTime(1);
        return false;
    }
    ...
    if (!skipEquipStanceCheck)
    {
        ...
        sLog->outString("DEBUG: Spell [%u] - Form [%X] - Need Form [%X] - Not Form [%X]", pSpellInfo->Id, formMask, pSpellInfo->Stances, pSpellInfo->StancesNot );
...
bool PlayerbotAI::HasPick()
{
    ...
    for (uint8 slot = INVENTORY_SLOT_ITEM_START; slot < INVENTORY_SLOT_ITEM_END; slot++)
    {
        sLog->outString("[%s's]backpack slot = %u",m_bot->GetName(),slot); // 23 to 38 = 16
    ...
    for (uint8 bag = INVENTORY_SLOT_BAG_START; bag < INVENTORY_SLOT_BAG_END; ++bag) // 20 to 23 = 4
    {
        ...
        if (pBag)
        {
            for (uint8 slot = 0; slot < pBag->GetBagSize(); ++slot)
            {
                sLog->outString("[%s's]bag[%u] slot = %u",m_bot->GetName(),bag,slot); // 1 to bagsize = ?
...
bool PlayerbotAI::TradeItem(const Item& item, int8 slot)
{
    ...
    /*if( (slot>=0 && slot<TRADE_SLOT_COUNT)  && m_bot->GetItem(tradeSlots)==NULL ) {
        tradeSlot = slot;
...
void PlayerbotAI::HandleCommand(const std::string &text, Player &fromPlayer)
{
 ...
 else if (text.size() > 2 && text.substr(0, 2) == "g " || text.size() > 4 && text.substr(0, 4) == "get ")
 {
     ...
     {
         sLog->outString("find: guid : %u entry : %u x : (%f) y : (%f) z : (%f) mapid : %d",guid, entry, x, y, z, mapid);
...
bool PlayerbotAI::DoLoot()
{
    ...
    if(!m_lootCurrent)
    {
        ...
        if(!m_lootCurrent)
        {
            sLog->outString("PlayerbotAI::DoLoot() error location #1, please report this error immediately!");
            return false;
        }
        ...
        if(m_bot->IsWithinDistInMap(object, INTERACTION_DISTANCE))
        {
            ...
            uint32 lootNum = loot->GetMaxSlotInLootFor(m_bot);
sLog->outString("[PlayerbotAI]: %s loot target 0x%08X got %d items", m_bot->GetName(), m_lootCurrent, loot->GetMaxSlotInLootFor(m_bot));
            ...
            for(uint32 l = 0; l < lootNum; ++l)
            {
                ...
                if(m_needItemList[item->itemid] > 0)
                {
sLog->outString("[PlayerbotAI]: %s LOOT needed item 0x%04X", m_bot->GetName(), item->itemid);
                    ItemPosCountVec dest;
                    if(m_bot->CanStoreNewItem(NULL_BAG, NULL_SLOT, dest, item->itemid, item->count) == EQUIP_ERR_OK)
                    {
                        Item *newitem = m_bot->StoreNewItem(dest, item->itemid,true, item->randomPropertyId);
